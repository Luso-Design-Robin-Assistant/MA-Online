...
<div class="mx-auto">
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4 min-h-screen h-full"><x-slot name="header">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Uploaden</h2>
        </x-slot>
        ...
        <div class="gallery js-flickity" data-flickity-options='{ "freeScroll": false, "wrapAround": true }'>
            @foreach($posts as $post)
                <div class="flex-col border w-max m-3.5">
                    <div class="ls-bar py-3 flex-row justify-between flex ">
                        <p class="px-4 py-2 items-center text-black flex">{{ $post->title }}</p>
                        <p class="px-4 py-2 items-center text-black flex">{{ $post->body }}</p>
                        <p class="px-4 py-2 items-center text-black flex">{{ $post->url }}</p>
